...
DB_PATH="/usr/local/omiinaya/filebrowser.db"
DEFAULT_PORT=8080
...
if [[ "${install_prompt,,}" =~ ^(y|yes)$ ]]; then
    ...
    mkdir -p /usr/local/omiinaya
    chown root:root /usr/local/omiinaya
    chmod 755 /usr/local/omiinaya
    touch "$DB_PATH"
...
[Service]
User=root
WorkingDirectory=/usr/local/omiinaya
ExecStartPre=/bin/touch /usr/local/omiinaya/filebrowser.db
ExecStartPre=/usr/local/bin/filebrowser config set -a "0.0.0.0" -p ${PORT} -d /usr/local/omiinaya/filebrowser.db
ExecStart=/usr/local/bin/filebrowser -r / -d /usr/local/omiinaya/filebrowser.db -p ${PORT}
Restart=always
